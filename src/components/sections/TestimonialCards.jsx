...
export function anonymize(name) {
  ...
  return name[0] + 'ㅇ'.repeat(name.length - 2) + name[name.length - 1];
}
...
export function renderCaption(caption, highlights) {
  ...
  for (const { hl, idx } of ranges) {
    ...
    parts.push(
      <span key={`h-${idx}`} className="font-extrabold text-bg-deep bg-yellow-200/70 px-0.5 rounded-[3px]">{hl}</span>
    );
