...
import { WEEKS5, PROGRAM5 } from '../../data/season5';

// 성공 관성이 이 시스템의 1번 축 — 나머지 둘보다 먼저 나온다.
const pillars = [
  {
    icon: '📈',
    title: '성공 관성 — 지속 가능',
    desc: `Day 1은 딱 ${PROGRAM5.startMinutes}분. 졸라 쉽게 시작해서 성공을 계속 쌓아가다 보면 관성이 붙어서 점점 쉬워져. 하루 1분씩만 늘려서 Day ${PROGRAM5.peakDay}에 ${PROGRAM5.peakMinutes}분.`,
  },
  {
    icon: '🔒',
    title: '채찍 — 강제성',
  ...
  },
  {
    icon: '🥕',
    title: '당근 — 보상',
    desc: '팀끼리 실시간 순위 경쟁. 1등 팀은 팀 상금으로 현금 10만원까지 받아.',
  },
];
    ...
    <section className="px-6 py-14 max-w-lg mx-auto">
      <AnimateOnScroll>
        ...
        <p className="text-text-secondary mb-8 leading-relaxed">
          의지가 아니야. <span className="text-text-primary font-bold">성공 관성 · 채찍 · 당근</span> — 지속 가능하게 만드는 단 3가지.
        </p>
      </AnimateOnScroll>
      ...
      <AnimateOnScroll className="mt-8">
        <div className="bg-bg-card rounded-3xl p-6 shadow-[0_12px_40px_rgba(0,0,0,0.15)]">
          ...
          <p className="text-card-ink-muted text-sm leading-relaxed mt-4 break-keep">
            첫날은 {PROGRAM5.startMinutes}분만 뛰면 돼. 하루 1분씩 늘어서 Day {PROGRAM5.peakDay}에 {PROGRAM5.peakMinutes}분,
            거기서부터는 계속 {PROGRAM5.peakMinutes}분 유지야. 마지막 날은 다 같이 5K 파이널 레이스.
          </p>
        </div>
      </AnimateOnScroll>
    </section>
