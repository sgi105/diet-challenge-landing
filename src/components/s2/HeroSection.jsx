...
export default function HeroSection({ onCTA }) {
  ...
  const countdownTarget =
    isPrereg ? COHORT2.preRegEnd
      : isInterlude ? COHORT2.officialOpen
        : COHORT2.officialDeadline;
  const countdownLabel =
    isPrereg ? 'PRE-APPLY · 사전신청 마감까지'
      : isInterlude ? 'OPENS IN · 정식 오픈까지'
        : 'DEADLINE · 마감까지';
          ...
          {!isClosed && (
            <div className="text-center my-5 animate-fade-up" style={{ animationDelay: '0.27s' }}>
              <p className="text-text-secondary text-xs sm:text-sm font-bold leading-relaxed">
                🏆 1등 팀 전원 <span className="text-text-muted">→</span> <span className="text-accent-green">DARIMATI 러닝화 · 60만원 상당</span>
              </p>
            </div>
          )}
